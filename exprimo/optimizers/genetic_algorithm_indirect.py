...
class GAIndirectOptimizer(BaseOptimizer):

    def __init__(self, population_size=100, mutation_rate=0.01, elite_size=20, steps=500, use_caching=False,
                 *args, **kwargs):
    ...
    def optimize(self, net_string, device_graph):
        ...
        def decode(genome):
            placement = [0] * n_groups
            for gene in genome:
                for i in range(gene[0], n_groups):
                    placement[i] = gene[1]

            return placement

        def calculate_fitness(genome):
            placement = decode(genome)
        ...
        def breed(parent1, parent2):
            split = random.randint(0, max(len(parent1) - 1, 0))
        ...
        def mutate(individual, mutation_rate):
            ...
            if random.random() < mutation_rate:
                ...
                i = random.randint(0, n_groups - 1)
                d = random.randint(0, n_devices - 1)
                for g in range(len(individual)):
                    if i == individual[g][0]:
        ...
        return json.dumps(apply_placement(net_string, decode(best_solution), groups))
