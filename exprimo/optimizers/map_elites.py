...
def _evaluate(individual, net_string, groups, device_graph, dimension_sizes, pipeline_batches=1, batches=1,
    ...
    device_mode = round((mode(individual) / len(device_graph.devices)) * dimension_sizes[0])
    used_devices = round(len((set(individual) - 1) / (len(device_graph.devices) - 1)) * dimension_sizes[1])

    comp_graph = apply_placement(net_string, individual, groups)
    ...
    score = 1 / evaluate_placement(comp_graph, device_graph,
...
class MapElitesOptimizer(BaseOptimizer):
    ...
    def optimize(self, net_string, device_graph):
        ...
        map = np.empty(self.dimension_sizes)
        ...
        def create_candidates(n, random=False):
            candidates = []
            if random:
                while len(candidates) < n:
                    candidates.append(generate_random_placement(len(groups), n_devices, allow_device_0=self.allow_cpu))
            else:
                # TODO: Implement random sampling from MAP
                pass

            return candidates

        for i in tqdm(range(0, self.steps, self.n_threads)):
            init_number = min(max(0, self.initial_size), self.n_threads)
            candidates = create_candidates(init_number, random=True)
            ...
            for result in eval_results:
                ...
                previous_elite = map[description[0], description[1], description[2]]
                if previous_elite is None or previous_elite[0] < score:
                    map[description[0], description[1], description[2]] = (score, individual)
